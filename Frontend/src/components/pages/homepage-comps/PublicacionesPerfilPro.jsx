...
import './style.css'
import { likePublication, dislikePublication } from './likesDislikes';
...
const PublicacionesPerfilPro = () => {
  ...
  const handleLike = async (publicationId) => {
    try {
      ...
      // Aquí puedes realizar cualquier acción adicional después de dar like
  ...
  const handleDislike = async (publicationId) => {
    try {
      await dislikePublication(publicationId);
      // Aquí puedes realizar cualquier acción adicional después de dar dislike
            ...
            Array.isArray(usuarioPublicaciones) && usuarioPublicaciones.map((publicacion, subIndex) => (
              <li className='publicacion-realizada' key={`${index}-${subIndex}`}>
                ...
                <div className="botones-comentario " style={{ marginTop: 12 }}>
                  <button onClick={() => handleLike(publicacion.id)}>Like</button>
                  <button onClick={() => handleDislike(publicacion.id)}>Dislike</button>
